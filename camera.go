...
func (c *Camera) castLevel(x int, grid [][]int, lvl *level, levelNum int, wg *sync.WaitGroup) {
	...
	if x == convergenceCol && drawStart <= convergenceRow && convergenceRow <= drawEnd {
		...
		if c.convergenceDistance == -1 || convergenceDistance < c.convergenceDistance {
			...
			c.convergencePoint = &Vector3{X: convergenceLine3d.X2, Y: convergenceLine3d.Y2, Z: convergenceLine3d.Z2}
		}
	}
...
func (c *Camera) SetPosition(pos *Vector2) {
	c.pos = pos
}
...
func (c *Camera) getAngleFromVec(dir *Vector2) float64 {
	return math.Atan2(dir.Y, dir.X)
}

// Get the dir vector from angle and fov length
func (c *Camera) getVecForAngleLength(angle, length float64) *Vector2 {
	return &Vector2{X: length * math.Cos(angle), Y: length * math.Sin(angle)}
}

func (c *Camera) getVecForAngle(angle float64) *Vector2 {
	return &Vector2{X: c.fovDepth * math.Cos(angle), Y: c.fovDepth * math.Sin(angle)}
}

// Get the plane vector from FOV based on dir vector
func (c *Camera) getVecForFov(dir *Vector2) *Vector2 {
...
func (c *Camera) GetConvergencePoint() *Vector3 {
